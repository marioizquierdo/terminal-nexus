...
| Campaign Level 1 — the ten-milestone sequence | CURRENT | Canon 2.8's campaign-first pivot, formalized at 2.9 into ten focused milestones at Mario's own request: building PERIMETER turned out to need most of the game's unbuilt systems at once (a menu, a campaign screen, a real Build Phase, the Pulse's player-facing moment, an economy, a Commander, cutscenes), so it is a sequence, not one contract. Milestones now live in their own [`../milestones/`](../milestones/) folder — trackers, not versioned canon. Currently at **Milestone 2 — Campaign Design**: deciding what PERIMETER contains before milestones 3-10 build against it. Q29 answered (Recall is the existing regroup rule); Q30-Q36 registered with recommendations | Confirm Milestone 2's design decisions with Mario, then open Milestone 3. [`../milestones/README.md`](../milestones/README.md) is the index; that folder's own CURRENT row is the authority |
