...
| Milestone 1B — quality and effects | CURRENT | Built and evidenced: eleven effects under the pure contract, four render tiers, an optional glyph pack, and a second fixture army for contrast. [`../evidence/gate-1b-report.md`](../evidence/gate-1b-report.md) concludes PASS on everything a test can answer, and explicitly not on the rest. **Fed back three times in one day** (2026-08-22/23, `npm run play`, all against the same unmerged branch — see the report's Sections 12 and 14, and its Section 14 note that round three was tested against `main`, which had none of this yet): a stuck-warning bug masking units placed inside impassable terrain since Gate 1A, death/blast timing and weight, a ranged kill's own target vanishing before its tracer, movement sped up twice (1.5x then 2x the original), a new post-kill movement-hold kernel rule, and four new questions registered (Q21 contrast, Q22 movement jitter, Q23 outposts, Q24 cell aspect ratio) | Decide how this branch reaches `main` — nothing is merged and no PR exists. Then watch it for real; accept or revise Gate 1B (and Gate 1A alongside it, still never separately closed) — not merely wait for a session to run out of fixes |
