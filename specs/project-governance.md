...
| Milestone 2 — Level 1: Perimeter | CURRENT | Canon 2.8's campaign-first pivot: Mario asked to build the campaign one level at a time rather than complete the Pulse horizontally first. PERIMETER (`campaigns.md` Section 4.2, already fully written) rebuilt as a real vertical slice — Build Phase, the unmodified Nexus Pulse kernel, a scripted opponent, the mission's own briefing/debrief, one unlock — reusing the existing disposable Citizen/Ravel fixture content rather than authoring a Commander Army. Five new questions registered (Q29-Q33), each with a recommendation this milestone already proceeds under | Open Gate 2A — the Build Phase loop. [`milestone-2-deterministic-pulse.md`](milestone-2-deterministic-pulse.md) is the contract |
| Backlog — completing the Pulse | GATED | What "Milestone 2" used to mean: routing, economy, production, visibility, and replay-format hardening, built horizontally. Superseded in *sequencing* by the campaign-first pivot, not in content — every contract and locked fixture is preserved verbatim in [`backlog-pulse-completion.md`](backlog-pulse-completion.md) | Pulled in by whichever campaign level's own contract first needs a specific piece of it — not authorized as a standalone gate |
| Milestone 3 — Build Phase and editor | GATED | Requires a complete kernel. Its content is now consulted as backlog per level rather than built as one horizontal pass — see its own file's backlog note | Lock placement rules before a level's contract actually needs the full editor |
| Milestone 4 — Citizens versus Ravels | GATED | Requires presentation, kernel, and builder proofs. Its roster and Commander Army authoring stay reserved; campaign levels build toward it on disposable fixture content until enough of them justify committing to it | Select the deliberately tiny integrated ruleset once enough levels justify it |
| Milestone 5 — human campaign fragment | GATED | Superseded in shape by the campaign-first pivot: the fragment it describes is now being built one level at a time, starting with Milestone 2. Still the reference for the belief ramp's later missions | Consult when authoring the level that reaches each later mission |
