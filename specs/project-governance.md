...
| Milestone 1B — quality and effects | CURRENT | Built and evidenced: eleven effects under the pure contract, four render tiers, an optional glyph pack, and a second fixture army for contrast. [`../evidence/gate-1b-report.md`](../evidence/gate-1b-report.md) concludes PASS on everything a test can answer, and explicitly not on the rest. **Watched a second time** (2026-08-22, `npm run play`): encouraging response plus a concrete list, acted on in the report's Sections 12-13 — two real bugs found and fixed (a stuck-warning bug that was masking units placed inside impassable terrain since Gate 1A), death/blast timing and weight corrected, movement sped up 1.5x, three new questions registered (Q21 contrast, Q22 movement jitter, Q23 outposts) | Watch it again; accept or revise Gate 1B (and Gate 1A alongside it, still never separately closed) — not merely wait for a session to run out of fixes |
