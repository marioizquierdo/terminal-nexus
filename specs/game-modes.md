...
## 5. What we learned from, and what each one proves — reference, not authority

Where an item could not be read directly by the session that wrote this (several sites were
unreachable from its network), the summary rests on search extracts and the author's prior knowledge
and says no more than that.

| Source | What it proves for Terminal Nexus |
| --- | --- |
| **Slay the Spire** — *Metrics Driven Design and Balance*, Anthony Giovannetti, GDC 2019 ([GDC Vault](https://www.gdcvault.com/play/1025731/-Slay-the-Spire-Metrics), [YouTube](https://www.youtube.com/watch?v=7rqfbvnO_H0)); card-reward mechanics ([wiki](https://slay-the-spire.fandom.com/wiki/Card_Rewards)) | The run-draft loop: pick one of three after every fight or skip; removal at a merchant; a rarity **pity offset** (starts at −5% rare, +1% per non-rare roll, resets on a rare, capped); ascension levels as the difficulty ladder; balance by pick rate and win rate with weekly updates and the caveat that metrics mislead when read naively |
| **Super Auto Pets** ([wiki, The Basics](https://superautopets.wiki.gg/wiki/The_Basics)) | Tier gating by turn — tier *X* unlocks at turn 2*X* − 1 — and a five-slot team: small deck, short run, deep replay. The closest existing loop to "build, then watch it fight, then draft" |
| **Mechabellum** ([Steam](https://store.steampowered.com/app/669330/Mechabellum/), [MonsterVine review](https://monstervine.com/2025/04/mechabellum-review/)) | The nearest cousin of Terminal Nexus's own match: supplies per round, place units, no control during the fight, temporary and permanent upgrades chosen between rounds. Its **Survival** mode is a single-player run — waves that change each round, four upgrade options per round, unit durability, difficulty tiers that lengthen the run. Proof that a build-then-watch RTS sustains a run structure |
| **Into the Breach** — design postmortem, Matthew Davis, GDC 2019 ([GDC Vault](https://www.gdcvault.com/play/1025772/-Into-the-Breach-Design)); persistence via the time-traveller pilot ([PCGamesN](https://www.pcgamesn.com/into-the-breach/permadeath)) | A grid tactics run where **clarity beat cool every time** — content that could not be conveyed was cut, which is this project's legibility law in another game's words. Between runs, exactly one pilot and the squads unlocked by achievements persist: options, not power |
| **Hades** — Greg Kasavin on roguelikes and narrative, GDC Podcast ep. 16 ([gdconf.com](https://gdconf.com/article/roguelikes-and-narrative-design-with-hades-creative-director-greg-kasavin-gdc-podcast-ep-16/)); the Mirror of Night ([wiki](https://hades.fandom.com/wiki/Mirror_of_Night)) | Story and runs coexist when death is part of the fiction and each run advances a small piece of narrative. For this project the lesson is the split Mario already drew: the Campaign carries the story; runs carry the interface's own voice and bark-sized texture, not chapters |
| **Teamfight Tactics** — pool sizes and shop odds by level ([MetaTFT](https://www.metatft.com/tables/shop-odds), [Dot Esports](https://dotesports.com/tft/news/teamfight-tactics-champion-pool-rolling-chances)) | Cost tiers (1–5) with per-level odds and finite pool sizes per tier: the fully quantified version of "tier gates when, rarity weights how often." More machinery than a single-player run needs; the shape is the same |
| **Mark Rosewater** — *Nuts & Bolts #4: Higher Rarities* and *Quite the Rarity*, Making Magic ([Wizards](https://magic.wizards.com/en/news/making-magic/nuts-bolts-higher-rarities-2012-02-27), [Wizards](https://magic.wizards.com/en/news/making-magic/quite-rarity-2018-03-12)) | The rarity doctrine in Section 4: uncommons help you win, rares just win; complexity belongs above common; two common effects together are often not common. The canonical public writing on rarity as a design tool, not a collectability tool |
| **Roguebook** — deckbuilding and roguelite design, Abrakam ([Game Developer](https://www.gamedeveloper.com/design/tackling-deckbuilding-design-in-abrakam-s-roguebook)) | A studio's account of fitting a deckbuilder to a roguelite structure; useful as a second worked example beside Slay the Spire (not readable from this session's network; listed for the next reader) |
| **Book** — *Procedural Storytelling in Game Design*, eds. Tanya X. Short and Tarn Adams, CRC Press 2019 ([Routledge](https://www.routledge.com/Procedural-Storytelling-in-Game-Design/Short-Adams/p/book/9781138595309)) | The reference for making runs *tell* something without authored chapters — characterization, world-building, and momentum from systems. Co-edited by Dwarf Fortress's author, which makes it the one book on this list written partly from inside an ASCII game |
