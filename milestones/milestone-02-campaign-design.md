...
> **A design pass ran on 2026-08-28, and Section 4 below is now a first draft under review.** Mario:
> "don't take the previous decisions seriously, they were generated by the previous agent based on the
> initial specifications as I was defining the project." What is actually undecided is narrower and
> more concrete than Section 4 assumes — **how the missions look, who the general is, and where to
> start**. The pass that addresses those lives in
> [`../evidence/milestone-02-campaign-design/`](../evidence/milestone-02-campaign-design/); start with
> that folder's own `README.md`, and see Section 7 below for what it contains. Five decisions came out
> of it and are waiting on Mario: **Q39** (where the campaign starts), **Q43** (what the Commander does
> for a living), **Q40** (fixed unlock or a choice), **Q41** (the lose-everything set-piece), and
> **Q42** (a second Commander).
...
- [x] the unit list above is confirmed against the actual fixture content — done in
      ...
      which also records two findings: the Citizens have no worker-producing building distinct from the
      barracks, and the raid should deliberately get no Grid Nexus even though one exists on the bench;
- [x] a real map layout exists for the first mission's Grid, terrain, and starting placements — a
      60 × 20 Grid with terrain rows drawn out, occupied tile ranges for every starting entity, and the
      Q33 off-axis constraint written down as something future edits must preserve;
- [x] the trigger-list shape for the scripted raid is written down precisely enough that milestone 6
      can implement it without a second design pass — including the simplification to **one verb**
      (`spawn`, not `spawn` plus `activate`) and the five determinism rules it needs;
- [x] Q29 is moved to Answered (done before this session);
- [ ] Q32 and Q33 are moved to Answered, citing this document — **held on purpose.** Both are specific
      to the raid-defence mission, and Q39 could still change which mission opens the campaign; closing
      them now would mean closing them twice. Recommended for closure in the same pass that answers
      Q39;
- [x] Q38 is explicitly proceeded-under-recommendation — the 60 × 20 size is chosen so the Grid scrolls
      at the 80 × 24 terminal floor and fits whole at the viewport maximum;
- [ ] Mario has looked at the decisions and either confirmed them or asked for a specific change —
      **this is now the whole remaining question.** Five decisions are waiting: Q39, Q43, Q40, Q41, Q42.

## 7. The design pass, 2026-08-28

Eight documents in
[`../evidence/milestone-02-campaign-design/`](../evidence/milestone-02-campaign-design/), written in
plain English for Mario rather than in this repository's own shorthand (AGENTS.md Section 5 now
requires that of anything he reads). Start with that folder's `README.md`.

| Document | What is in it |
| --- | --- |
| `README.md` | The short version of everything, and the five decisions waiting on Mario |
| `where-to-start.md` | Four ways to open the campaign, compared, plus the "you lose the first battle" idea explored properly → **Q39**, **Q41** |
| `who-leads-and-what-happens.md` | What the Commander does for a living, how she is introduced, how the war escalates over six missions, where a second Commander fits → **Q43**, **Q42** |
| `what-a-mission-looks-like.md` | The five beats every mission shares, the build-phase telegraph constraint, and a fully worked first mission — real map, real coordinates, real enemy schedule |
| `unlocking-and-teaching.md` | The unlock system, the eight alternatives deliberately refused, the one-new-idea-per-mission rule, and how to teach the two decisions an autobattler actually has → **Q40** |
| `what-other-games-do.md` | The research: nine games, one mechanism each, cited |
| `player-and-developer.md` | Every recommendation checked against both experiences, with the five rows where one wins at the other's cost |
| `notes-for-the-next-agent.md` | Cross-references, canon touchpoints, and loose ends — the internal shorthand the other documents deliberately avoid |

The process write-up closing this pass is
